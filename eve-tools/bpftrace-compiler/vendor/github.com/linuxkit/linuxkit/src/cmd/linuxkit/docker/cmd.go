...
func HasImage(ref *reference.Spec, arch string) error {
	log.Debugf("docker inspect image: %s", ref)
	cli, err := Client()
	if err != nil {
		return err
	}
	img, err := InspectImage(cli, ref)
	if err != nil {
		return err
	}
	if img.Architecture != "" && img.Architecture != arch {
		return fmt.Errorf("architecture not matching")
	}
