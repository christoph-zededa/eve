...
func main() {
	...
	compileCmd := &cobra.Command{
		...
		SuggestFor: []string{},
		Short:      "compile only",
		Example:    fmt.Sprintf("%s amd64 docker.io/lfedge/eve-kernel:eve-kernel-amd64-v6.1.38-generic-fb31ce85306c-gcc opensnoop.bt opensnoop.aot", execName),
	...
	runSSHCmd := &cobra.Command{
		Use:        "run-ssh <host:port> <bpftrace script>",
		Aliases:    []string{"rs"},
		SuggestFor: []string{},
		Short:      "run bpftrace script on host via ssh",
		Example:    fmt.Sprintf("%s 127.1:2222 opensnoop.bt", execName),
	...
	runHTTPCmd := &cobra.Command{
		Use:        "run-http <host:port> <bpftrace script>",
		Aliases:    []string{"rh"},
		SuggestFor: []string{},
		Short:      "run bpftrace script on host via http debug",
		Example:    fmt.Sprintf("%s 127.1:6543 opensnoop.bt", execName),
		Args:       cobra.ExactArgs(2),
		Run: func(cmd *cobra.Command, args []string) {
