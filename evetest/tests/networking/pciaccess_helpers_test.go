...
func parsePCIAccessTrace(output string) ([]pciAccess, error) {
	...
	for _, line := range strings.Split(output, "\n") {
		...
		if err := json.Unmarshal([]byte(line), &msg); err != nil {
			return nil, fmt.Errorf("unparseable tracer output line %q: %w", line, err)
		}
		switch msg.Type {
		case "lost_events":
		...
		if err := json.Unmarshal(msg.Data, &text); err != nil {
			return nil, fmt.Errorf("unparseable printf data in tracer output line %q: %w",
				line, err)
		}
		...
		if fields := bpftraceOpenLineRE.FindStringSubmatch(text); fields != nil {
			...
			if err != nil {
				return nil, fmt.Errorf("unparseable tracer output %q: %w", text, err)
			}
			flags, err := strconv.ParseUint(fields[7], 16, 32)
			if err != nil {
				return nil, fmt.Errorf("unparseable flags in tracer output %q: %w", text, err)
			}
