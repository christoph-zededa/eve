...
const (
	...
	// StatLineSize is the average size of a log line in bytes.
...
func MemoryPSICollector(collectorCtx context.Context, log *base.LogObject) error {
	...
	// it will be truncated. We keep no more than approximately 24 hours of data. It's ~6.3 MB.
	var thresholdSize int64 = StatLineSize * StatLinesPerHour * 24
	...
	// hour.
	linesToKeep := StatLinesPerHour * 23
	...
	for {
		select {
		case <-ticker.C:
			if err := manageStatFileSize(log, statFile, thresholdSize, linesToKeep); err != nil {
...
// threshold size and keeping only the last linesToKeep lines.
func manageStatFileSize(log *base.LogObject, statFile *os.File, thresholdSize int64, linesToKeep int) error {
	// Check the file size
	...
	if fileInfo.Size() > thresholdSize {
		log.Errorf("file size is still greater than the threshold size\n")
		return fmt.Errorf("file size is still greater than the threshold size")
	}
