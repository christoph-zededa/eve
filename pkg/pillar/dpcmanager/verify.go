...
func (m *DpcManager) verifyDPC(ctx context.Context) (status types.DPCState) {
	...
	intfStatusMap, tracedProbes, err := m.ConnTester.TestConnectivity(
		m.deviceNetStatus, withNetTrace, types.NetTraceFolder)
...
func (m *DpcManager) checkIfMgmtPortsHaveIPandDNS() bool {
	...
	for _, port := range mgmtPorts {
		numAddrs, err := types.CountLocalAddrAnyNoLinkLocalIf(m.deviceNetStatus, port)
		if err != nil {
			m.Log.Errorf("CountLocalAddrAnyNoLinkLocalIf failed for %s: %v",
				port, err)
			continue
		}
		if numAddrs < 1 {
