...
type fbDriver struct {
	name      []byte // Driver name
	sysfsPath string // Driver's sysfs path
}
...
func (fbDrv fbDriver) unbind() error {
	return os.WriteFile(path.Join(fbDrv.sysfsPath, "unbind"), fbDrv.name, 0)
}
...
func getFBDrvs() ([]fbDriver, error) {
	...
	fbDrvs, err := fs.Glob(sysfb, "*-framebuffer")
	...
	var drvs []fbDriver
	for _, v := range fbDrvs {
		drv := fbDriver{
			name:      []byte(fmt.Sprintf("%s.0", v)),
			sysfsPath: path.Join(fbdir, v),
		}
...
func unbindAllFramebufferDrivers() error {
...
func rebindFramebufferDrivers() error {
	fbDrvs, err := getFBDrvs()
	if err != nil {
		return err
	}
...
func (vt vtInfo) unbind() error {
	return os.WriteFile(path.Join(vt.sysfsPath, "bind"), []byte{'0'}, 0)
}

// Bind Virtual Terminal (VT) console
func (vt vtInfo) bind() error {
	return os.WriteFile(path.Join(vt.sysfsPath, "bind"), []byte{'1'}, 0)
}
...
// update the active tty device global variable
func guessCurrentTty() int {
	activeTty, err := os.ReadFile("/sys/devices/virtual/tty/tty0/active")
	...
	match := re.FindSubmatch(activeTty)
	if len(match) != 2 {
...
func listVTConsolesFromSysfs() ([]vtInfo, error) {
	vtdir := "/sys/class/vtconsole"

	vtDrvs, err := filepath.Glob(filepath.Join(vtdir, "vtcon*"))
	if err != nil {
		return nil, err
	}

	re, err := regexp.Compile("vtcon([0-9]+)$")
	if err != nil {
	...
	for _, v := range vtDrvs {
		...
		// error
		name, err := os.ReadFile(path.Join(vtdir, v, "name"))
		if err != nil {
			log.Errorf("Cannot read VT %v driver's name: %v", v, err)
		}
		bind, err := os.ReadFile(path.Join(vtdir, v, "bind"))
		if err != nil {
			log.Errorf("Cannot read VT %v bind state: %v", v, err)
		}
		bound, err := strconv.ParseBool(strings.TrimSpace(string(bind)))
		if err != nil {
...
func detachAllActiveVTs() error {
	...
	deviceVTs, err = listVTConsolesFromSysfs()
	if err != nil {
...
func restoreActivatedVTs() error {
	if len(deviceVTs) == 0 {
